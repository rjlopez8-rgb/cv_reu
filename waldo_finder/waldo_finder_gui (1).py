...
class WaldoFinderApp:
    ...
    def analyze_current(self):
        ...
        wf.RED_RANGES = [
            (np.array([0,   s_min, 70]), np.array([10,  255, 255])),
            (np.array([168, s_min, 70]), np.array([180, 255, 255])),
        ]
        wf.WHITE_RANGE = (np.array([0, 0, v_min]), np.array([180, 80, 255]))
        # Keep skin's brightness floor in step with white's so faces still
        # register on darker scans
        wf.SKIN_RANGE = (np.array([0, 25, max(120, v_min - 20)]),
                         np.array([22, 120, 255]))
        ...
        if self.candidates:
            ...
            self.status.config(
                text=f"{len(self.candidates)} candidate(s) — best is a "
                     f"'{top['kind']}' match, score {top['score']:.1f}")
        else:
    ...
    def refresh_list(self):
        ...
        for rank, c in enumerate(self.candidates):
            ...
            kind = c.get("kind", "torso")
            # mark the detector type so you can trust glasses-based hits more
            tag = {"fused": "[head+shirt]", "head": "[glasses]",
                   "torso": "[stripes]"}.get(kind, "")
            txt = f"#{rank+1}  {tag}  score {c['score']:.1f}"
